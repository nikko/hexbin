	subroutine hbin(x, y, cell, cnt, wcnt, xcm, ycm, size, shape,
     *                  rx, ry, bnd, n, weights, cellid)
...
c                       and an optional weights argument returns the 
c                       sum of weights not counts
c       Copyright (2004) Nicholas Lewin-Koh and Martin Maechler

	implicit none

	integer n, nc, cell(*), cnt(*), bnd(2), cellid(*)
c       cellid(*) and weights(*) : length 1 or n
	double precision x(n), y(n), xcm(*),ycm(*), rx(2), ry(2), size
        double precision shape, weights(n), wcnt(*)
