...
function(x, y, hexC = hexcoords(dx, dy, n = 1), dx, dy=NULL,
	     fill = 1, border = 0, hUnit = "native", draw=TRUE, ...)
{
    ...
    if(hexC$no.sep) {
        ...
        if(!is.null(hUnit)) {
            grid.polygon(x = unit(rep.int(hexC$x, n) + rep.int(x, n6),hUnit),
                         ...
                         id.lengths = n6,
                         gp = gpar(col= border, fill= fill),draw=draw)
        }
        else {
...
function(dat, style = c("colorscale", "centroids", "lattice",
         ...
         def.unit = "native",
         verbose = getOption("verbose"), draw=TRUE)
{
    ...
    hexgrob <- switch(style,
	   ...
	   "nested.centroids" = {
	       hexpolygon(xnew, ynew, hexC,
			  ...
			  border = pen[,1],draw=draw)
	   }
	   )
    if(style%in%c("constant.col","colorscale")) return(invisible(hexgrob))
    ...
    hexgrob2 <- switch(style,
	   "centroids" = ,
	   "lattice" = {
	       grid.polygon(pltx, plty, default.units=def.unit, id=NULL,
			    ## density = density,
			    id.lengths= n6,
			    gp=gpar(fill = pen, col = border), draw=draw)
	   },
	   "nested.lattice" = ,
	   "nested.centroids" = {
	       hg <- grid.polygon(pltx, plty, default.units=def.unit, id=NULL,
			    id.lengths= n6,
			    gp=gpar(fill = pen[,2],
			    ## density = density,
			    col=if(!is.null(border)) border else pen[,2]), draw=draw)
               
               gList(inner=hg,outer=hexgrob)
	   })
    return(invisible(hexgrob2))
}
