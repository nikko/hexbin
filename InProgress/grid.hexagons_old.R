hexcoords <- function(dx, dy = NULL, n = 1, sep = NA) {
    if(is.null(dy)) dy <- dx/sqrt(3)
    if(is.null(sep))
      return(list(x = rep.int(c(dx, dx,	0, -dx, -dx, 0), n),
	          y = rep.int(c(dy,-dy, -2*dy, -dy,  dy, 2*dy), n),
                  no.sep=TRUE)) 
    else return(list(x = rep.int(c(dx, dx, 0, -dx, -dx,	 0, sep), n),
...
{
    ## Purpose: draw hexagon polygon()'s  around  (x[i], y[i])_i
    ## Author: Martin Maechler, Date: 26 Jul 2004, 14:56
...
### Replace hexagons() from ../../hexbin/R/hexbin.R  by grid-version:
### unfinished!
grid.hexagons <-
	 ...
	 verbose = getOption("verbose"))
{
    ...
    if(!is(dat,"hexbin")) stop("first argument must be a hexbin object")
    ...
    switch(style,
	   centroids = ,
	   lattice = {if(is.null(pen))pen <- rep.int(1, length(cnt))
    ...
    #seven <- rep.int(7:7, n)
    six <- rep.int(6:6, n)
    pltx <- rep.int(hexC$x, n) * rep.int(radius, six) + rep.int(xnew, six)
    plty <- rep.int(hexC$y, n) * rep.int(radius, six) + rep.int(ynew, six)
    switch(style,
	   "centroids" = ,
	   "lattice" = {
	   ...
	   "nested.centroids" = {
	       grid.polygon(pltx, plty, default.units=def.unit, id=NULL,
             ...
             #polygon(pltx, plty,
		       #density = density, col = pen[,2],
		       #border = if(!is.null(border)) border else pen[,2])
	   })
