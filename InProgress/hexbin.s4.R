...
hexbin <-
    function(x, y = NULL, weights=NULL, rotation=0. xbins = 30, shape = 1,
             ...
             Triangulate=FALSE)
{
    ...
    ## Lots of duplication here, do we want to have a "less safe"
    ## version that does not copy the data so many times?
    x <- xy$x
    y <- xy$y
    n <- length(x)
    if(missing(weights)){
    ...
    lmax <- jmax * imax
    dims <- c(imax,jmax)
    ans <- .Call("hbin", x, y, weights, shape, rotation, xbins, xbnds, ybnds, dims, n, IDs, Triangulate)
    ## hbin(SEXP x, SEXP y, SEXP swts, SEXP shape, SEXP rot,
    ...
    ##           cell = integer(lmax),
    ##           cnt = integer(lmax),
    ##           wcnt = double(lmax), ##if(wflag) as.double(lmax) else as.double(-1.0),
    ##           xcm = double(lmax),
    ##           ycm = double(lmax),
    ##           xbins = as.double(xbins),## size
    ##           shape = as.double(shape),
    ##           xbnds = as.double(xbnds),## rx
    ##           ybnds = as.double(ybnds),## ry
    ##           dim = as.integer(c(imax, jmax)),## bnd
    ##           n = as.integer(n),
    ##           ##wflag = if(!wflag) as.integer(-1) else 1
    ...
    new("hexbin",
	cell = ans$cell, count = ans$cnt, wcount=ans$wcnt,
...
setClass("hexbin",
	 representation(cell = "integer", count = "numeric", wcount = "numeric",
